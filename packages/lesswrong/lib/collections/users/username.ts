...
export const usernameIsTaken = async (currentUser: DbUser, username: string) => {
  const existingUser = await Users.findOne({username})
  return existingUser && existingUser._id !== currentUser._id
}

export const assertUsernameIsNotTaken = async (currentUser: DbUser, username: string) => {
  if (await usernameIsTaken(currentUser, username)) {
