    ...
    if (editorRef.current) {
      ...
      const cleanupSuccessForm = context.addToSuccessForm((result: any, form: any, submitOptions: any) => {
        ...
        if (editorRef.current && !submitOptions?.redirectToEditor) {
          ...
          // There's the visible state that the user can see, and there's
          // the state tracked in the Editor component, and they're different.
          // The Editor component state is updated after a debounce period of
          // not typing (i.e. the autosaveInterval, currently 3 seconds). If they
          // submit the form without ever having paused for 3 seconds, the Editor
          // component state will still be empty, so if we set it to empty here,
          // it won't trigger the onChange handler, which is where the debounced
          // state change is triggered, so the visible-to-user state will remain
          // and then get auto-saved. So if we've submitted the form and the
          // contents seems to be empty, we set it to a dummy value and *then*
          // set it to empty, to make sure that onChange handler gets triggered.

          if (contents.value.length === 0) {
            wrappedSetContents({
              contents: {type: initialEditorType, value: 'a'},
              autosave: false,
            });
          }
